...
**CRITICAL — 抓取文档前 MUST 先判定来源：本地 `.md` 用 Read；飞书链接（含 feishu.cn / larksuite）用 `lark-cli docs +fetch --api-version v2 --doc <url>`。判不准就问用户，禁止猜。**
**CRITICAL — 拆需求 MUST 一个需求一份独立 md，写入工作区 `requirements/<需求id>.md`，frontmatter 声明 `projects`（涉及的项目名列表）。禁止把多个需求塞进一份文档。**
...
**CRITICAL — `+kb` 首次为项目建库时 MUST 往 `<项目>/CLAUDE.local.md` 写知识库加载约定块（说明 KNOWLEDGE.md 在哪、怎么读、怎么更新），并确保 `.gitignore` 含 `CLAUDE.local.md`（暂不进 git）。改 CLAUDE.local.md 属敏感操作，仅在用户授权后追加,已有约定块则跳过。**
...
│   ├── knowledge/src/.../KNOWLEDGE.md   ← 镜像源码树的知识库
...
```
/work-flow <文档路径 | 飞书链接>     # 等价 +split，从文档进入全流程
/work-flow +init                     # 初始化工作区：建 requirements/、探测子项目
/work-flow +split <文档 | 链接>      # 抓取文档 + 拆成一个个需求 md
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `<文档路径 \| 飞书链接>` | 需求来源，本地 `.md` 或飞书文档 URL | 必填 |
| `<需求id \| 需求md>` | requirements/ 下的需求编号或文档路径 | 必填 |
...
1. **判源 + 抓取**：本地 `.md` 用 Read；飞书链接用 `lark-cli docs +fetch --api-version v2 --doc <url>`。
2. **拆分**：把文档内容拆成一个个独立、可单独设计与开发的需求。粒度规则见 split-guide。
3. **定位项目**：`+init` 已探测的候选项目里，判断每个需求涉及哪些项目，填 `projects`。判不准就问。
4. **落盘**：每需求写 `requirements/<需求id>.md`（模板见 split-guide）。已存在则提示 覆盖/更新/预览。
...
1. **读需求**：Read 对应 `requirements/<需求id>.md`，取 `projects`。
...
1. **读方案**：Read `<项目>/docs/design/<需求id>.md`，按方案实现。
2. **按方案开发（小步快跑）**：贴合项目现有约定；复用调研阶段确认的现有能力。每写完一个完整功能/模块就主动提示用户提交 git（提示,用户确认才提,不自动）。
3. **持续更新知识库（收尾必做显式判定）**：开发完成后显式判定——改动是否影响已记录知识（漂移）、或读码问出新隐藏约束。有则**提议更新**对应 KNOWLEDGE.md（用户确认才写）,无则明说原因,不可静默跳过(详见 dev-guide 二)。
4. **测试用例（条件化）**：项目已有测试机制才补用例，贴合现有写法；无测试机制则跳过,禁止为单需求硬塞框架（用户要搭建才引入）。
5. **人工验收**：实现完成后交人工验收，skill 不自行判定通过。
...
0. **首次为该项目建库 → 写加载约定**：项目根无 `CLAUDE.local.md` 知识库约定块时,征得同意后追加(模板见 knowledge-guide 七),并确保 `.gitignore` 含 `CLAUDE.local.md`（暂不进 git）。已有则跳过。
...
| 按方案开发、知识库漂移定位与持续更新节奏 | [`references/dev-guide.md`](references/dev-guide.md) |
